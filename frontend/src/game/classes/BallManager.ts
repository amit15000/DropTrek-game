...
export class BallManager {
  ...
  getColor(index: number) {
    ...
    if (index < 8 || index > this.sinks.length - 8) {
      return { background: "#ff7f00", color: "white" };
    }
    if (index < 11 || index > this.sinks.length - 11) {
      return { background: "#ffbf00", color: "black" };
    }
    if (index < 14 || index > this.sinks.length - 14) {
      return { background: "#ffff00", color: "black" };
    }
    if (index < 17 || index > this.sinks.length - 17) {
